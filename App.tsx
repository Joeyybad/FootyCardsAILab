...
export default function App() {
  ...
  // New state for editing card image URL
  ...
  const swapToOfficialImage = (id: string, newUrl: string) => {
    setCollection(prev => prev.map(card => card.id === id ? { ...card, imageUrl: newUrl } : card));
    setSelectedCard(prev => prev && prev.id === id ? { ...prev, imageUrl: newUrl } : prev);
  };

  const handleUpdateImageManual = () => {
    if (selectedCard && newAssetUrl.trim()) {
      setCollection(prev => prev.map(card => card.id === selectedCard.id ? { ...card, imageUrl: newAssetUrl.trim() } : card));
      setSelectedCard(prev => prev ? { ...prev, imageUrl: newAssetUrl.trim() } : null);
  ...
  const handleScout = async () => {
    ...
    setScoutProgress('Grounded Data Synchronization...');
    ...
    try {
      ...
      if (uploadedBase64) {
        ...
        setScoutProgress(`Forging Cinematic Asset...`);
        try {
          finalImageUrl = await generatePlayerImage(data.name || 'Unknown', data.club || 'Free Agent', data.rarity as PlayerRarity);
        } catch (imgErr: any) { 
          finalImageUrl = data.suggestedImageUrl || ''; 
        }
      }

      // Normalization Layer: Ensure rarity is Title Case
      ...
      const newCard: PlayerCard = {
        ...
        suggestedImageUrl: data.suggestedImageUrl,
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row transition-colors duration-500 ${theme === 'dark' ? 'bg-[#0a0a0c] text-white' : 'bg-slate-50 text-slate-900'}`}>
      {/* Sidebar Navigation: The Lab Control */}
      <aside className={`w-full md:w-[22rem] p-8 glass border-r flex flex-col gap-8 h-screen sticky top-0 z-30 overflow-y-auto ${theme === 'dark' ? 'bg-[#0a0a0c]/80 border-white/10' : 'bg-white/95 border-slate-200 shadow-2xl'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-black opacity-40 tracking-widest leading-none">Stadium Protocol</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] font-black uppercase tracking-widest opacity-50 flex items-center gap-2"><Search size={14} /> Intelligence Engine</h2>
          ...
          <div className="relative">
            <input 
              type="text" 
              placeholder="Scout by player name..." 
              value={searchTerm} 
          ...
          {isAdvanced && (
            <div className={`p-6 rounded-[1.5rem] border space-y-4 animate-in fade-in slide-in-from-top-2 duration-300 ${theme === 'dark' ? 'bg-blue-900/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase opacity-60 flex items-center gap-1.5"><LinkIcon size={12}/> Asset Overwrite</label>
                <input type="text" value={customImageUrl} onChange={(e) => setCustomImageUrl(e.target.value)} className={`w-full rounded-xl px-4 py-3 text-xs border bg-transparent outline-none focus:border-blue-500 ${theme === 'dark' ? 'border-white/10' : 'border-slate-300'}`} placeholder="Bypass AI portraits..." />
              </div>
              <div onClick={() => fileInputRef.current?.click()} className="w-full py-5 border-2 border-dashed rounded-[1.25rem] flex flex-col items-center cursor-pointer hover:bg-blue-500/10 transition-colors border-blue-500/30">
                ...
                <span className="text-[10px] uppercase font-black text-blue-500">Local Treasury Upload</span>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-5 rounded-[1.5rem] border transition-all hover:border-blue-500/50 ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
              <p className="text-[9px] uppercase font-black opacity-40 mb-1">Squad Assets</p>
              <p className="text-2xl font-oswald leading-none">{collection.length}</p>
            </div>
            <div className={`p-5 rounded-[1.5rem] border transition-all hover:border-blue-500/50 ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
              <p className="text-[9px] uppercase font-black opacity-40 mb-1">Squad Value</p>
              <p className="text-2xl font-oswald text-blue-500 leading-none">€{(totalValue / 1000000).toFixed(1)}M</p>
            </div>
          </div>

          <div className="space-y-3">
            <h2 className="text-[10px] font-black uppercase tracking-widest opacity-50 flex items-center gap-2"><Filter size={14} /> Treasury Filters</h2>
            <button 
            ...
            >
              All Athletes <span>{collection.length}</span>
            </button>
      ...
      <main className="flex-1 p-8 md:p-16 overflow-y-auto h-screen relative">
        <div className="max-w-7xl mx-auto h-full">
          {/* Pack Opening Experience (UX Design doc) */}
          {isRevealing && selectedCard ? (
            <div className="flex flex-col items-center justify-center h-full animate-in zoom-in-95 fade-in duration-1000">
              <div 
              ...
              >
                ...
                <div className="absolute -bottom-20 w-full text-center">
                   <p className="text-sm font-black uppercase tracking-[0.4em] text-blue-500 animate-pulse">Intelligence Validated</p>
                   <p className="text-[10px] uppercase font-bold opacity-40 mt-3">Tap to reveal athlete portrait</p>
                </div>
              </div>
            </div>
          ) : isComparisonMode && comparisonSelection.length === 2 ? (
            /* Battle Arena Mode (UX Design doc) */
            ...
            /* Detailed View Mode */
            <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-16 animate-in slide-in-from-bottom-8 duration-700 items-start">
              <div className="flex flex-col items-center gap-8 sticky top-8">
                ...
                <div className="flex flex-col gap-3 w-full max-w-sm">
                  {/* Manual Asset Override Section */}
                  {isEditingUrl ? (
                    <div className="flex flex-col gap-2 p-4 rounded-2xl glass border border-blue-500/30 animate-in fade-in slide-in-from-top-1">
                      <input 
                        type="text" 
                        value={newAssetUrl} 
                    ...
                    >
                      <LinkIcon size={16} /> Manual Asset Override
                    </button>
                  )}

                  {selectedCard.suggestedImageUrl && selectedCard.imageUrl !== selectedCard.suggestedImageUrl && (
                    <button 
                      onClick={() => swapToOfficialImage(selectedCard.id, selectedCard.suggestedImageUrl!)} 
                      className="py-4 px-8 rounded-2xl bg-blue-600/10 text-blue-500 text-[11px] font-black uppercase flex items-center justify-center gap-3 border border-blue-600/30 hover:bg-blue-600/20 transition-all transform hover:scale-[1.02]"
                    >
                      <RefreshCw size={16} /> Force Official Asset Link
                    </button>
                  )}
              ...
              <div className="space-y-12 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className={`p-8 rounded-[1.5rem] glass border shadow-sm ${theme === 'dark' ? 'bg-white/[0.03] border-white/10' : 'bg-white/70 border-slate-200'}`}>
                    ...
                    <p className="text-[10px] uppercase font-bold opacity-30 mt-4 tracking-widest leading-none">Estimated Asset Value</p>
                  </div>
                  <div className={`p-8 rounded-[1.5rem] glass border shadow-sm ${theme === 'dark' ? 'bg-white/[0.03] border-white/10' : 'bg-white/70 border-slate-200'}`}>
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] opacity-50 mb-4">Official Roster</h3>
                    ...
                    <p className="text-[10px] uppercase font-bold opacity-30 mt-4 tracking-widest leading-none">Registered Registry Location</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  >
                    <ExternalLink size={18} /> Direct Intel Access
                  </button>
                </div>
                ...
                {selectedCard.groundingSources && selectedCard.groundingSources.length > 0 && (
                  <div className="space-y-6 pt-6 border-t border-current opacity-10">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] opacity-40 flex items-center gap-3"><Globe size={16} className="text-blue-500" /> Verifiable Data Sources</h3>
            ...
            <div className="space-y-12">
              <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b pb-8 border-current opacity-10">
                 <div>
                   <h2 className="text-5xl font-oswald uppercase leading-none mb-2">Validated Treasury</h2>
                   <p className="text-[11px] uppercase font-black tracking-[0.4em] opacity-40">Monitoring {filteredCollection.length} Verified Athletes</p>
                 </div>
                 {isComparisonMode && (
                   <div className="px-6 py-4 bg-blue-600/10 text-blue-600 rounded-3xl border border-blue-600/20 text-[10px] font-black uppercase flex items-center gap-3 animate-pulse">
                     <Swords size={16} /> Select 2 Cards to Battle ({comparisonSelection.length}/2)
                   </div>
                 )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                {filteredCollection.length > 0 ? (
                  ...
                  <div className="col-span-full h-[35rem] flex flex-col items-center justify-center opacity-10 border-4 border-dashed rounded-[2rem] border-current">
                    ...
                    <h3 className="font-oswald text-4xl uppercase tracking-tighter">Treasury Unpopulated</h3>
